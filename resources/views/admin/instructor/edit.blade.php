    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-card.card-default class="static">
                ...
                <x-form action="{{ route('dashboard.admin.instructor.update', $instructor->id) }}"
                    class="md:grid md:grid-cols-2 gap-4" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <input type="hidden" name="oldImage" value="{{ $instructor->photo }}">
                    @if ($instructor->photo)
                        <div class="avatar">
                            <div class="w-12 rounded-xl">
                                <img src="{{ asset('storage/instructor/photo/' . $instructor->photo) }}" />
                            </div>
                        </div>
                    @endif
                    <div class="mt-4">
                        ...
                        <x-input.input-file id="photo" class="mt-1 w-full" type="file" name="photo"
                            :value="old('photo', $instructor->photo)" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-input.text-input id="name" class="mt-1 w-full" type="text" name="name"
                            :value="old('name', $instructor->name)" required autofocus autocomplete="name" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-input.select-input id="gender" class="mt-1 w-full" type="text" name="gender" required
                            ...
                            <option value="Laki - Laki"
                                {{ old('gender', $instructor->gender) == 'Laki - Laki' ? ' selected' : ' ' }}>Laki -
                                Laki
                            </option>
                            <option value="Perempuan"
                                {{ old('gender', $instructor->gender) == 'Perempuan' ? ' selected' : ' ' }}>
                                Perempuan</option>
                    ...
                    <div class="mt-4">
                        ...
                        <x-input.text-input id="phone" class="mt-1 w-full" type="number" name="phone"
                            :value="old('phone', $instructor->phone)" required autofocus autocomplete="phone" />
