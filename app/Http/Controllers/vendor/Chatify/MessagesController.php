...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        $fetch = User::where('id', $request['id'])->with(['instructor', 'participant'])->first();
        if ($fetch) {
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = User::join('ch_messages', function ($join) {
            $join->on('users.id', '=', 'ch_messages.from_id')
                ->orWhere('users.id', '=', 'ch_messages.to_id');
        })
            ->where(function ($q) {
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('id', '!=', Auth::user()->id)
            ...
            ->orWhereHas('participant', function ($query) use ($input) {
                $query->where('name', 'LIKE', "%{$input}%");
            })
            ->orWhereHas('instructor', function ($query) use ($input) {
                $query->where('name', 'LIKE', "%{$input}%");
